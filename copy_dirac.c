...
//         target->fermion[x] = info->mass * fermion->fermion[x];
//         #endif
//         #endif

//         #ifndef PI_INTERACTION
//         #ifdef SIGMA_INTERACTION
//         target->fermion[x] = (info->mass + lat->sigmamass[x]) * fermion->fermion[x];
//         #endif
//         #endif

//         #ifdef SIGMA_INTERACTION
//         #ifdef PI_INTERACTION
//         target->fermion[x] = (info->mass + lat->sigmamass[x] + (I_UNIT * lat->epsilon_pi[x] * lat->pimass[x])) * fermion->fermion[x];
//         #endif
//         #endif
//     }
...
//         target->fermion[x] += (0.5) * fermioncoor->eta_pp[COOR(x, d)] * lat->dlink[COOR(fermioncoor->site[x], d)].link * fermion->fermion[fermioncoor->fermionpp[COOR(x, d)]];
//         // - (0.5) \eta_mu(x) * U_mu(x-mu) * delta_{x, x-mu}
//         target->fermion[x] -= (0.5) * fermioncoor->eta_mm[COOR(x, d)] * conj(lat->dlink[COOR(lat->mm[COOR(fermioncoor->site[x], d)], d)].link) * fermion->fermion[fermioncoor->fermionmm[COOR(x, d)]]; 
//     }
// }
